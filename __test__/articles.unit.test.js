...
test('filter without any parameters', async t =>
  t.throws(articles.filter(), Error, 'calling filter wihtout data should throw an error')
);
...
test('filter with mock data, without options', async t => {
  ...
  const actual = [filtered.next().value, filtered.next().value];
...
test('filter with mock data, with *from* option set to an items *time_added*', async t => {
  const filtered = articles.filter(MOCK_DATA);
  const next = filtered.next();
  const actual = [next.value];
  const expected = ['46524476'];
  t.true(next.done, 'filter should be done here');
...
test('filter with mock data, with *from* option set to an items *time_added* + 1', async t => {
  const filtered = articles.filter(MOCK_DATA, {
    from: new Date((parseInt(MOCK_DATA['1409914364'].time_added, 10) * 1000) + 1)
  });
  const next = filtered.next();
  const actual = [next.value];
  const expected = ['46524476'];
